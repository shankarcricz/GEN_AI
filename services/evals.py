...
test_case_data = {
  ...
  "test_cases": [
    {"id": "R01", "category": "resume_only", "question": "What is my current role and company?", "expected_contains": ["Comcast", "Engineer 2"], "assertion": "all(kw.lower() in response.lower() for kw in expected_contains)", "ground_truth_chunk_id": ["resume:80c1bfcbade2e314"], "notes": "direct fact lookup"},
    ...
    {"id": "R12", "category": "resume_only", "question": "What is my current salary?", "expected_behavior": "refuse_or_not_in_context", "assertion": "not any(c.isdigit() for c in response) or 'not available' in response.lower() or \"don't have\" in response.lower() or 'no information' in response.lower()", "ground_truth_chunk_id": [], "notes": "out-of-scope: not on resume, tests against hallucinated numbers. Empty ground truth is intentional \u2014 no chunk should be a valid 'hit' here."},
    # {"id": "R13", "category": "resume_only", "question": "What is my marital status?", "expected_behavior": "refuse_or_not_in_context", "assertion": "'not' in response.lower() and ('available' in response.lower() or 'mentioned' in response.lower() or 'information' in response.lower())", "ground_truth_chunk_id": [], "notes": "out-of-scope, personal, not on resume. Empty ground truth intentional."},
    ...
    # {"id": "R15", "category": "resume_only", "question": "What visualization or mapping work have I done?", "expected_contains": ["OpenStreetMap", "geospatial"], "assertion": "any(kw.lower() in response.lower() for kw in expected_contains)", "ground_truth_chunk_id": ["resume:80c1bfcbade2e314"]},

    {"id": "J01", "category": "jd_only", "question": "How many years of experience does this JD require?", "expected_contains": ["4"], "assertion": "'4' in response", "ground_truth_chunk_id": ["jd:Required:None"]},
    ...
    {"id": "J08", "category": "jd_only", "question": "Does this JD mention team size or reporting structure?", "expected_behavior": "refuse_or_not_in_context", "assertion": "'not' in response.lower() and ('mentioned' in response.lower() or 'specif' in response.lower() or 'available' in response.lower())", "ground_truth_chunk_id": [], "notes": "ambiguous/absent info \u2014 tests against over-refusal AND against fabrication; real JD text has no such detail. Empty ground truth intentional."},
    # {"id": "J09", "category": "jd_only", "question": "What evaluation-related skills does this JD value?", "expected_contains": ["evaluation", "unit tests", "eval"], "assertion": "any(kw.lower() in response.lower() for kw in expected_contains)", "ground_truth_chunk_id": ["jd:Preferred:None"]},
